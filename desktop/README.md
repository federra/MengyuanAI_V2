...
右上角月亮/太阳按钮切换浅色与深色，默认保持原浅色。使用 `app/dark-theme.css` 的深色限定覆盖与通用组件变量，不改原浅色 CSS；视频、图片不使用反色滤镜。偏好通过 localStorage 和同主机 Cookie 保存，兼容桌面重启端口变化；首屏脚本在绘制前恢复主题。`tests/theme-preference.mjs` 验证偏好逻辑，`desktop/test-theme.mjs` 使用独立测试数据库对比 0.1.63 与 0.1.64 的浅色计算样式并验证真实 Chromium 切换、刷新记忆与截图。
