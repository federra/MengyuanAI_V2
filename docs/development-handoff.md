...
- **0.1.72 发布准备（2026-09-16）**：用户已授权递增版本、合入 main、推送 GitHub，并发布 Mac/Windows。包含豆包修复、故事操作区左右分组、10分钟模型等待、分镜导入兼容、Skill多格式/ZIP及独立Mac下载渠道。Windows安装包、Apple Silicon及Intel独立应用已构建；Apple Silicon启动测试通过，Mac更新入口与Skill真实UI测试通过。发布结果以本节后续记录及服务器清单为准。
