...
更新日期：2026-09-24（Asia/Shanghai）。目的：让新的 Codex 对话理解现有工程并接续开发新模块。本文记录交接时状态，不保证未来进程、远端或发布目录仍相同；开工应核对 Git 和实际文件。本文不包含服务器密码或模型密钥。

## 一、接手时最重要的事实

- **2026-09-24 Mac本机0.1.102预览（未公开发布）**：新项目剧本约4430字、资产155项，旧路径将所有资产描述送入模型，且空结果/截断只给笼统提示。现将剧本按片段生成，仅带相关资产并压缩描述，逐段校验后合并；长度截断时进一步拆分片段，全部完成前不改项目。三类文本生成显示非阻塞中央提示，允许切换页面；分镜视频列扩大预览、按钮底部对齐、尾帧引用标签、模型详情及一步生成，拖拽排序增加鼠标释放兜底，行间新增更醒目。0.1.101实机已通过鼠标拖动换位并恢复原顺序；0.1.102已修正默认窗口下模型列横向溢出，完成构建并打开桌面端复验布局。未调用真实模型额度，分镜输出质量仍待真实项目验证。

- **2026-09-24 Mac本机0.1.100预览（未公开发布）**：模型生成列改成模型独占第一行，比例、时长、生成按钮在第二行；桌面窗口明确resizable并降低最小尺寸，初始宽度最多1450且相对屏幕工作区留白。分镜列宽再次压缩，保证常见桌面窗口能完整看到最右侧生成按钮。分镜排序手柄改用指针捕获拖动，指针抬起时也按最终位置完成快速拖放，保留方向键微调。类型检查、定向lint、Web构建、主进程语法检查通过；本机0.1.100已启动、确认分镜布局和Mac原生“缩放窗口”。窗口边缘拖拽未实测；自动化鼠标拖动分镜手柄未能确认换位，需人工复核。未上传公开更新、未制作Windows包。

- **2026-09-24 Mac本机0.1.95预览（未公开发布）**：分镜表视频操作图标悬停提示，交换删除视频与更多的位置；行拖拽排序（手柄支持上下方向键）、首尾及行间悬停添加；勾选后显示批量删除并确认，原操作列改为模型生成设置。临时预览0.1.93曾发现序号换行，0.1.94发现窄窗口下横向溢出，均已在0.1.95修正。Windows安装目录选项仍仅源码，未打包发布。

- **2026-09-24 Windows安装选项（仅源码，未打包发布）**：`desktop/build/windows-config.cjs` 将 NSIS 从一键安装改为分步安装，启用目录选择并明确首次安装默认创建桌面快捷方式；现有用户范围、升级位置及 userData 归属不变。尚未生成新的 Windows 安装包，Windows 实机安装界面待验收。

- **2026-09-24 Mac源码预览0.1.92（未打包、未公开发布）**：先将此前UI迭代以提交 `ab72050` 推送GitHub main，再继续本轮调整。剧本页移除“分场编辑（可选）”；分镜页只保留组合视图，点击镜头序号/更多中的参数入口打开轻量弹窗，不再切换到独立详细页。表格列顺序改为分镜序号、视频、提示词、台词、资产绑定、操作；压缩行高，视频操作缩为图标，资产以分类文字标签显示并可预览已有图片。类型检查、定向lint、Web构建及真实源码桌面布局验证通过；未打包安装版，也未提交本轮新增调整。

- **2026-09-24 Mac本机0.1.91已启动（未公开发布）**：分镜工作区顶部流程导航保持与前序环节一致的紧凑字号/图标；标题缩小，导入分镜移到标题右侧，移除重复保存与预览按钮，导出改为普通描边按钮。角色、场景、道具、批量配音、批量站位图整合为“资产管理”五页签，豆包插件保留独立入口；分镜表头滚动悬浮于流程导航下方。类型检查、定向lint、Web构建通过；真实桌面窗口已核对版号、布局、页签切换、选中分镜传入批量站位图及悬浮表头，未提交真实生成任务。独立目录 `desktop/release/v0.1.91/mac/`，旧目录、用户数据及在线更新渠道保留。

- **2026-09-24 Mac本机0.1.90已启动（未公开发布）**：创意页固定生成3个故事方案，点击后立即进入故事工作区等待结果，候选卡仅在故事页展示；移除创意页版本数筛选和下方候选区。故事栏默认加宽，分界线支持鼠标拖动与键盘微调，卡片顶部缩至34px。剧本页移除重复工具栏及资产摘要，标题右侧导入文本进入同一个导入弹窗，移除独立导入正文/阅读预览；正文默认按场次和角色名格式化展示，点击切换回原纯文本编辑，“分场编辑（可选）”保留。类型检查、定向lint、Web构建和真实桌面布局/拖动/导入验证通过；生成跳转代码已核对，未调用真实模型消费额度。独立目录 `desktop/release/v0.1.90/mac/`，旧目录、用户数据与在线渠道保留。

- **2026-09-24 Mac本机0.1.86已启动（未公开发布）**：故事工作区左侧卡片列表固定为约3张的可视高度，保留常显滑块，可滚动查看后续故事，批量生成按钮固定在列表下方。关闭“导入创作内容”时清除启动状态，流程导航切换也清除启动状态，修复点击“故事”偶发误开导入弹窗。类型检查、定向lint、Web构建及真实桌面窗口验证通过；实机完成“打开导入→关闭→切换创意→返回故事”回归，未再弹窗。独立目录为 `desktop/release/v0.1.86/mac/`，桌面快捷入口已添加，旧版本、用户数据和在线更新渠道保留。

- **2026-09-24 Mac本机0.1.81已启动（未公开发布）**：故事工作区缩小标题，移除顶部重复工具栏和标题旁旧导入/预览按钮；“导入文本”移至标题右侧，打开时默认故事类型，保留粘贴、TXT/MD文件、预览与确认；故事卡列表独立滚动，底部“AI 再来3个”使用原批量方案生成并追加。移除左侧“回到创意生成方案”与右下单篇生成/另存按钮。类型检查、定向lint、Web构建和Electron隔离启动通过，正式窗口已确认故事页、导入弹窗和版本0.1.81；旧目录、用户数据及在线渠道保留。

- **2026-09-24 Mac本机0.1.79已启动（未公开发布）**：创作中心顶部提供项目切换、改名入口及尺寸/风格标签，保存按钮移入顶栏；创意页移除重复项目行和工具行，缩小工作区标题，五步导航压缩并滚动悬浮，四项参数与一句话创作合并成一张卡片。类型检查、定向lint、Web构建及Electron隔离启动通过；正式窗口已确认0.1.79、创意布局与项目切换菜单。旧目录、用户项目及在线渠道保留。

- **2026-09-24 Mac本机0.1.78已启动（未公开发布）**：导航类目在原色系内增强对比，桌面顶部移除重复折叠按钮（窄屏保留必要入口）；项目中心新建按钮左对齐，卡片增加已保存内容进度标签与视频静止画面/图片封面，失败回退。回归测试、类型检查、lint、构建、浏览器素材夹具和Electron隔离启动通过；正式0.1.78窗口已打开项目中心，确认真实视频封面和状态正常。旧目录与数据保留，线上渠道未变，详见[实施与验证](project-center-ui-2026-09-24.md)。

- **2026-09-24 Mac本机0.1.77已启动（未公开发布）**：用户要求打开修改后的桌面端，已升本机版本并重新构建，使用共享白名单生成独立目录 `desktop/release/v0.1.77/mac/`。隔离Electron启动检查通过（登录页、未登录API401、渲染器无Node访问），9个客户端JS与最新dist逐字节一致；正式窗口已确认显示v0.1.77、四类导航和新名称。桌面新增0.1.77快捷入口，旧目录与资料保留；未制作Windows包或更新线上渠道。详见[导航实施记录](navigation-ui-2026-09-24.md)。

- **2026-09-24 左侧导航分组（仅源码，未发布）**：按用户批准稿改为创作、知识、设置、经营四组共十个入口；创作工作台更名创作中心，管理员后台更名管理后台（仍仅超管可见），Skill入口统一为skill中心。保留全部原有配色，仅调整布局和字体；增加侧栏顶部收起/展开图标按钮，收起后保留页面图标及原有悬停名称。剪辑输出不再单列于侧栏，原创作流程及资产页导出入口保留。类型检查、定向lint、构建及浏览器隔离导航检查通过；尚未重新打包桌面、升版或发布，正在运行的旧桌面不会自动更新。见[实施记录](navigation-ui-2026-09-24.md)。

- **2026-09-23 UI交互评审原型（非正式实现）**：按“调整当前版本”方向完成[低保真原型](prototypes/workflow-ui-adjustment/README.md)，覆盖创意→故事→剧本→分镜及可选素材→成品导出。成品导出沿用现有五类能力，不新增内置MP4；原型仅本地样例/模拟，未改业务源码、未升版发布。路线估算与正式实施步骤见[设计方案](superpowers/specs/2026-09-23-film-workflow-ui-adjustment-design.md)和[实施清单](superpowers/plans/2026-09-23-film-workflow-ui-adjustment.md)，真实改造计时尚未执行。

- **最新发布：Windows 0.1.76（2026-09-22）**：功能提交 `fcae0ef` 已推送main；Windows安装包及更新清单已上线，四文件服务器大小/SHA-512、公开HTTPS清单与Range206通过。包含下列分镜/资产改进，Mac渠道保持0.1.75。Windows实机待验收；详见[发布记录](release-0.1.76.md)。下方“未发布”均为本次发布前的历史阶段记录。

- **2026-09-22 外部分镜导入补修（未发布）**：完整storyboards格式先确定性转换，保留原台词/提示词/资产/音色和sourceMetadata；未知格式仍AI一次转换并严格复核。用户code.json已只读重放通过6段60秒7条台词零模型调用；隔离真实桌面检查预览及确认应用通过，最新入口 `desktop/test-results/storyboard-import-KEu9go/启动修复预览.command`，详见[实施记录](storyboard-conversion-assets.md)。

- **2026-09-22 源码调整（未发布）**：分镜生成/导入/编辑应用统一校验，不合格使用默认文本模型转换一次再校验；角色默认三视图及Skill；资产管理固定表头、清除引用、分离预览与应用/替换、图片目录重新选择历史图片。见 [实施记录](storyboard-conversion-assets.md)。单元/模拟接口、类型检查、lint、构建及隔离Mac桌面选图回归通过；Windows实机待验收。按用户纠正，已撤掉自定义目录UI，恢复原生文件选择窗口；Mac/Windows共用动态项目目录并解析真实路径。后续真实Mac窗口复验确认路径正确但原生分栏横向停在左侧，已在用户原生窗口选择图标显示，关闭重开后图片直接可见；这是本机原生视图修正，不冒称跨设备代码修复。最新本地入口 `desktop/test-results/image-directory-native-6rr053/启动修复预览.command`。线上版本仍0.1.75，本轮未升版、提交推送或发布。

- **最新发布：Windows及Mac 0.1.75（2026-09-21）**：功能提交`cd35f41`已推送main；Windows x64、Mac Apple Silicon/Intel三个包均已发布，服务器SHA-512和公开HTTPS Range206核对通过，双端渠道均0.1.75。包含保存、旧草稿恢复、分镜时间轴及全部图片尺寸修复，Mac同步获得0.1.74界面与登录改进。Mac ARM独立包启动及包内恢复保存隔离验收通过，Windows/Intel实机仍待完成；签名与发布详情见[0.1.75记录](release-0.1.75.md)。下方未发布记录为历史阶段；原运行中的Mac预览未强制替换。
